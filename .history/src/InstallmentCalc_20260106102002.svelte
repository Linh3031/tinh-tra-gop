<script>
    // Component: Tính Trả Góp v7 (Final: Header Logo + Firebase + LocalStorage)
    import { onMount, onDestroy } from 'svelte';
    import { incrementCounter, subscribeCounter } from './firebase';
    ...
    onMount(() => {
        // 1. NẠP LỊCH SỬ TỪ TRÌNH DUYỆT (LocalStorage)
        const savedHistory = localStorage.getItem('installment_history');
        if (savedHistory) {
            try {
        ...
        // 2. Lắng nghe Firebase Counter
        try {
            unsubscribe = subscribeCounter((val) => {
                usageCount = val;
            });
        } catch (e) {
            console.warn("Chưa cấu hình Firebase đúng cách");
        }
    });
    ...
    $: {
        if (history) {
            localStorage.setItem('installment_history', JSON.stringify(history));
        }
    }
...
<div class="h-full flex flex-col bg-slate-50 relative overflow-hidden">
    ...
    <div class="bg-white px-3 py-2 shadow-sm z-20 flex items-center justify-between border-b border-slate-200">
        <div class="flex items-center gap-2">
            <img src="/logo-header.jpg" alt="Logo" class="w-8 h-8 rounded-lg object-cover shadow-sm border border-slate-100">
            <h1 class="text-base font-black text-indigo-700 uppercase tracking-tight">Tính Trả Góp</h1>
        </div>

        <div class="flex items-center gap-1 bg-slate-100 px-2 py-1 rounded-full border border-slate-200" title="Lượt sử dụng toàn hệ thống">
            <span class="material-icons-round text-[12px] text-slate-400">equalizer</span>
            <span class="text-[10px] font-black text-slate-600">{usageCount.toLocaleString('vi-VN')}</span>
        </div>
    </div>

    <div class="bg-white px-2 pt-1 pb-0 z-10 flex gap-2 border-b border-slate-200">
        <button 
