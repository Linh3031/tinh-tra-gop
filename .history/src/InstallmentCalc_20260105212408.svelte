<script>
    // Component: Tính Trả Góp v3 (Combo Calculator + History Checkbox)
    ...
    function saveToHistory() {
        ...
        const newItem = {
            ...
            term: termMonths,
            diff: diffPrice,
            selected: false // Default chưa chọn
        };

        history = [newItem, ...history].slice(0, 10);
        // Reset checkbox các dòng khác nếu muốn, hoặc giữ nguyên
    }
...
<div class="h-full flex flex-col bg-slate-50 relative overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto pb-64 scroll-smooth">
        ...
        {#if activeTab === 'history'}
            <div class="p-2 space-y-2 animate-fadeIn pb-20">
                {#if history.length === 0}
        ...
        <div class="fixed bottom-0 left-0 w-full bg-white border-t border-indigo-200 shadow-[0_-4px_20px_rgba(0,0,0,0.15)] z-[60] rounded-t-xl animate-slideUp pb-6 bg-indigo-50">
            <div class="p-3 max-w-md mx-auto">
                ...
                <div class="flex gap-3">
                    <div class="flex-1 bg-white p-2 rounded shadow-sm border border-indigo-100">
                        <div class="text-[9px] font-bold text-slate-400 uppercase">Tổng Trả Trước</div>
                        <div class="text-xl font-black text-teal-600">{formatCurrency(comboTotalPrepaid)}<small>đ</small></div>
                    </div>
                    <div class="flex-1 bg-white p-2 rounded shadow-sm border border-indigo-100">
                        <div class="text-[9px] font-bold text-slate-400 uppercase">Tổng Góp/Tháng</div>
                        <div class="text-xl font-black text-indigo-600">{formatCurrency(comboTotalMonthly)}<small>đ</small></div>
                    </div>
                </div>
            </div>
        </div>
